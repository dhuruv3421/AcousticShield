...
# ──────────────────────────────────────────────────────────────
#  MAP BUILDER  ← FIXED: use FeatureGroup properly, ensure markers added to map
# ──────────────────────────────────────────────────────────────
def create_threat_map(threat_history: list, center_lat: float, center_lon: float) -> folium.Map:
    ...
    geo_threats = [
        e for e in threat_history
        if e.get("latitude") is not None and e.get("longitude") is not None
    ]

    # Determine map center and zoom
    if geo_threats:
        avg_lat = sum(e["latitude"]  for e in geo_threats) / len(geo_threats)
        avg_lon = sum(e["longitude"] for e in geo_threats) / len(geo_threats)
    ...
    # Add threat markers directly to map (not inside a FeatureGroup that may not render)
    for entry in geo_threats:
        lat   = entry["latitude"]
        lon   = entry["longitude"]
        cat   = (entry.get("threat_category") or "unknown").lower()
        ...
        popup_html = (
            ...
            'border-radius:4px 4px 0 0;font-weight:bold">&#9888; THREAT DETECTED</div>'
        ...
        marker_color = FOLIUM_MARKER_COLORS.get(cat, "gray")

        folium.Marker(
            location=[lat, lon],
            popup=folium.Popup(popup_html, max_width=280),
            tooltip=f"THREAT: {label} ({conf}%)",
            icon=folium.Icon(
                color=marker_color,
...
def render_result_card(entry, threshold=THREAT_THRESHOLD):
    ...
    dur_note   = f' &middot; {entry["duration"]}s' if entry.get("duration") else ""
    ...
    st.markdown(
        ...
        f'{entry.get("time","")}{dur_note} &middot; {entry.get("source","")}</span>{cat_pill}</div>'
        f'{badge}</div>'
...
def render_active_threat_alert(entry):
    ...
    <div class="alert-banner">
      <div class="alert-title">THREAT DETECTED &mdash; {cat.upper()}</div>
...
if selected_tab == "📂 File / Upload":
    if not st.session_state.model_loaded:
        st.warning("Model is not loaded yet. Check that the model file exists.")
    else:
            ...
            if st.session_state.mic_running:
                ...
                <div class="card" style="text-align:center;padding:24px">
                  ...
                  <div style="font-size:12px;color:#5a7a5a;margin-top:8px;
                    ...
                    Interval: {interval}s &middot; Chunks: {mic_tot}</div>
...
# ══════════════════════════════════════════════════════════════
    ...
    if not filtered:
        ...
        st.markdown(
            ...
            f'Showing {len(filtered)} &middot; '
            f'<span style="color:#c62828">{n_thr} threats</span> &middot; '
...
# ══════════════════════════════════════════════════════════════
#  TAB 5 – SAVED CLIPS  ← FIXED: use st.columns for metadata instead of inline HTML spans
# ══════════════════════════════════════════════════════════════
    ...
    if clip_files:
        ...
        st.markdown(
            ...
            f'<span style="color:#c62828">{len(clip_files)}</span> clip(s) &nbsp;&middot;&nbsp; '
            f'{total_kb:.1f} KB &nbsp;&middot;&nbsp; '
        ...
        for fn, fp, mtime in clip_files:
            ...
            # Determine category/color/confidence
            if matched_entry:
                cat       = (matched_entry.get("threat_category") or
                             clip_meta.get("threat_category") or "unknown")
                color     = CATEGORY_COLORS.get(cat, "#78909c")
                ...
                cat       = clip_meta.get("threat_category") or "unknown"
                color     = CATEGORY_COLORS.get(cat, "#78909c")
                conf      = int(float(clip_meta.get("threat_score", 0)) * 100)
                det_label = clip_meta.get("threat_label", "")
            ...
            if cat and cat != "unknown":
                cat_badge_html = (
                    f'<span style="background:#f5f5f5;border:1.5px solid {color};'
                    f'color:{color};font-family:Share Tech Mono,monospace;font-size:11px;'
                    f'padding:3px 12px;border-radius:20px;font-weight:700">'
                    f'{cat.upper()}</span>'
                )

            # Build label + confidence HTML
            label_html = ""
            if det_label:
                label_html = (
                    f'<span style="font-family:Share Tech Mono,monospace;font-size:12px;'
                    f'color:#c62828;font-weight:600">{det_label}</span>'
                    f'&nbsp;<span style="font-family:Share Tech Mono,monospace;font-size:12px;'
                    f'color:{color};font-weight:700">@ {conf}%</span>'
                )
            ...
            if c_lat is not None and c_lon is not None:
                ...
                coord_html = (
                    ...
                    f'font-size:10px;color:#1565c0">'
                    f'&#128205; {float(c_lat):.5f}, {float(c_lon):.5f}&nbsp;&nbsp;'
                    f'<a href="{maps_url}" target="_blank" '
                    f'style="color:#1565c0;text-decoration:underline">Google Maps &#8599;</a>'
                    f'</div>'
                )

            # Render entire card as one complete HTML block
            st.markdown(
                f'<div style="background:#fff8f8;border:2px solid {border_color};'
                ...
                f'<span style="font-family:Share Tech Mono,monospace;font-size:11px;color:#1a2e1a;min-width:140px">'
                f'&#128197; {friendly}</span>'
                f'{cat_badge_html}'
                f'<span style="flex:1">{label_html}</span>'
                f'<span style="font-family:Share Tech Mono,monospace;font-size:10px;color:#5a7a5a;white-space:nowrap">'
                f'&#9201; {dur_str}</span>'
                f'</div>'
                # Row 2: file path
                f'<div style="font-family:Share Tech Mono,monospace;font-size:10px;color:#7a8a7a;'
                f'word-break:break-all">&#128193; {fp}</div>'
                # Row 3: coordinates
                f'{coord_html}'
                f'</div>',
                unsafe_allow_html=True,
...
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        f'Total: {len(kw_data)} categories &middot; {total_kw} keywords &middot; '
        f'auto-saved to threat_keywords.json</div>',
        unsafe_allow_html=True,
    )

# ══════════════════════════════════════════════════════════════
#  TAB 7 – THREAT MAP  ← FIXED: markers now added directly to map
# ══════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        f'{geo_icon} {geo_label}: {cur_lat:.5f}, {cur_lon:.5f} &nbsp;&middot;&nbsp; '
        f'<span style="color:#c62828">{len(th)}</span> total threats &nbsp;&middot;&nbsp; '
        f'<span style="color:#1565c0">{len(geo_threats)}</span> on map</div>',
        unsafe_allow_html=True,
    )
    ...
    try:
        ...
        if clicked_data and isinstance(clicked_data, dict) and geo_threats:
            ...
            if c_lat is not None and c_lng is not None:
                ...
                st.markdown(
                    ...
                    f'{closest.get("threat_label","Unknown")} &mdash; {cat.upper()}</div>'
                    f'<div style="font-size:14px;color:#2d4a2d">'
                    f'<b>Confidence:</b> {conf}% &nbsp;&middot;&nbsp; '
                    f'<b>Time:</b> {closest.get("time","")} &nbsp;&middot;&nbsp; '
                    f'<b>Source:</b> {closest.get("source","")} &nbsp;&middot;&nbsp; '
                    f'<b>Coords:</b> {closest["latitude"]:.5f}, {closest["longitude"]:.5f}'
                    f'{"&nbsp;&middot;&nbsp; Clip saved" if closest.get("saved") else ""}'
                    f'</div>'
        ...
        try:
            ...
            st.info(f"Installed: streamlit-folium {_sfv} · folium {_fv}")
        except Exception:
            st.info("Run: pip install streamlit-folium==0.20.0 folium==0.17.0")

    if not geo_threats:
        st.info(
            "No geo-tagged threats yet. "
            "Run mic monitoring or file analysis to see threats on the map. "
            "Your current location marker (blue dot) is shown on the map."
        )