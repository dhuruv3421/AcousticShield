...
def init_state():
    defaults = dict(
        ...
        mic_interval=3,
        # Track which tab the user is on to avoid switching away on rerun
        active_tab=0,
    )
...
def check_threat(predictions, threshold=None):
    """Returns the highest-confidence threat found (if any)."""
...
# Active threat alert (non-tab-switching — acknowledged inline)
...
#  TABS
# ──────────────────────────────────────────────────────────────
tab_file, tab_mic, tab_log, tab_timeline, tab_saved = st.tabs([
    "📂  File / Upload",
...
# ══════════════════════════════════════════════════════════════
#  TAB 2 – MIC MODE
#  NOTE: st.rerun() inside this tab will re-render the whole
#  page but Streamlit keeps the active tab via JS state,
#  so the user stays on the Mic tab as long as they clicked it.
# ══════════════════════════════════════════════════════════════
with tab_mic:
    if not st.session_state.model_loaded:
        ...
        # Poll mic results and display — stays within this tab
        if st.session_state.mic_running and hasattr(st.session_state, "_result_q"):
            ...
            while not rq.empty():
                ...
                if entry["is_threat"]:
                    ...
                    # Update the active threat alert at top of page
                    # but DO NOT switch tabs — the user stays on Mic tab
                    st.session_state.active_threat = entry
                    st.session_state.threat_history.insert(0, entry)
                else:
                    st.session_state.stats["safe"] += 1
...
with tab_timeline:
    ...
    if not th:
        ...
        for entry in th[:20]:
            ...
            """, unsafe_allow_html=True)
# ══════════════════════════════════════════════════════════════
#  TAB 5 – SAVED THREAT AUDIO
# ══════════════════════════════════════════════════════════════
with tab_saved:

    st.markdown("#### 🎧 Saved Threat Audio Clips")

    os.makedirs(THREAT_SAVE_DIR, exist_ok=True)

    audio_files = [
        f for f in os.listdir(THREAT_SAVE_DIR)
    ...
    if not audio_files:
        ...
        <div style="
            ...
            color:#4fc3f7;
            margin-bottom:15px">
            Total Saved Clips: {len(audio_files)}
        </div>
        """, unsafe_allow_html=True)

        for file_name in audio_files:

            file_path = os.path.join(THREAT_SAVE_DIR, file_name)
